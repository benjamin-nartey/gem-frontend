...
const getSelectedRoleNames = (
  ...
  return selectedIds.map(
    (id) => options?.find((option) => option.id === id)?.name || id
  );
};
...
const MultiSelectorTrigger = forwardRef<
  HTMLDivElement,
  {
    ...
    roleOptions: { id: string; name: string }[]; // Define the prop type
  } & React.HTMLAttributes<HTMLDivElement>
>(({ className, children, roleOptions, ...props }, ref) => {
  ...
  const selectedRoleNames = getSelectedRoleNames(value, roleOptions); // Now roleOptions is available

  return (
    <div
      ref={ref}
      className={cn(
        "flex flex-wrap gap-1 p-1 py-2 ring-1 ring-muted rounded-lg bg-white w-full",
        {
      ...
      {selectedRoleNames.map((name, index) => (
        ...
        >
          ...
          <button
            ...
            onClick={() => onValueChange(value.filter((_, i) => i !== index))} // Remove by ID
          >
            <span className="sr-only">Remove {name} option</span>
